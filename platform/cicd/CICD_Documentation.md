...
Every credential this project needs — DockerHub, GitHub, AWS, ArgoCD, kubeconfig — lives only in `.env` (or `docker/jenkins.env`), both git-ignored, and is layered into the Jenkins container purely as environment variables at container start. `casc/jenkins.yaml` then turns those environment variables into entries in Jenkins' own built-in credentials store (`${DOCKERHUB_USERNAME}`, `${GITHUB_TOKEN}`, etc.) using `${VAR:-}` substitution, so a blank/unset variable produces an empty credential that a pipeline using it will fail on, rather than breaking Jenkins startup entirely. No secret value is ever hardcoded in `jenkins.yaml`, a `Jenkinsfile`, or a Docker image layer — which is also why that file is explicitly safe to commit.
